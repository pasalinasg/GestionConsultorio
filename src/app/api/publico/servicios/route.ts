...
export async function GET(request: Request) {
  ...
  const { data, error } = await db
    ...
    .select(
      "id,precio,profesional_id,profesionales(id,nombre_completo,descripcion),servicios(id,nombre,descripcion,modalidad,duracion_minutos)",
    )
  ...
  return NextResponse.json(
    {
      servicios: (data ?? []).map((item: Record<string, unknown>) => {
